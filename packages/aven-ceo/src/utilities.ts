...
import { INK_SCALE, RADIUS_SCALE, TINT_SCALE, TRACKING_SCALE, TYPE_SCALE } from './design.js'
import { APP_ROLES, CONTRAST_INK, CREAMS, ROLES, SITE_ROLES, SURFACES, TONES } from './tokens.js'
...
/**
 * Declarations, or a nested block keyed by a selector containing `&`.
 *
 * Nesting is native CSS, not a preprocessor step — `divide-y` genuinely needs
 * `& > * + *`, and writing it any other way would mean a second mechanism.
 */
type Decl = { [key: string]: string | Decl }
 ...
 */
export function appliedDecl(name: string): Decl | null {
 ...
 * reported as unknown.
 */
export function utilityCss(classes: Iterable<string>, known: Iterable<string> = []): UtilityResult {
