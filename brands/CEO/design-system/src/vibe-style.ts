...
import { vibeTokens } from './tokens.js'

/**
 * The brand, as a `StyleDef` the vibe StyleEngine can consume.
 *
 * IT LIVES HERE NOW. It was `@avenos/aven-ui`, a package in avenOS whose only
 * remaining job was this seam: avenCEO's palette wired into a shape the engine
 * understands. Its own header said where it belonged — "brand-style.ts is
 * avenCEO data expressed as code, so it belongs in @myavenceo/aven-ceo beside
 * the tokens it already reads from. When it moves, this package disappears
 * entirely." It moved; the package is deleted.
 *
