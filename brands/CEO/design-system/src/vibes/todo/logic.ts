...
/**
 * The slice of a loaded state machine this program reads.
 *
 * The full `Machine` (states, transitions, guards, contracts, the `.pl`
 * clause database) belongs to the consumer that parses the `.pl` file — the
 * brand does not own a Prolog reader. Only three members ever reach the
 * sandbox, so the brand states exactly those and stays structurally
 * compatible with the richer host type: any `Machine` is a `TodoMachine`.
 */
export interface TodoMachine {
	/** Every status column the board renders, in order. */
	states: string[]
	/** The legal status-to-status moves the reducer gates on. */
	statusMoves(): { from: string; to: string }[]
	/** The board button's forward walk: from-status to next-status. */
	cycles: { from: string; to: string }[]
}

/** The machine, injected as a data prelude in front of the behaviour. */
export function composeTodoProgram(machine: TodoMachine): string {
	const prelude =
