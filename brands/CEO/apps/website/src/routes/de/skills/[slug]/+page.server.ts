...
/* A server load rather than the universal one it replaces — see the English
 * route: the footer render belongs to the build, not the client bundle. */
export const load: PageServerLoad = async ({ params }) => {
	const skill = loadSkill(params.slug, 'de')
	if (!skill) throw error(404, 'Skill nicht gefunden')
	return { skill, footerHtml: await footerHtml('de') }
}
